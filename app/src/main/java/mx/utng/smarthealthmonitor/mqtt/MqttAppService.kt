...
import mx.utng.shared.mqtt.MqttConfig
import mx.utng.shared.mqtt.FcMessage
import mx.utng.shared.mqtt.TvMessage
...
class MqttAppService(
    ...
    private val onFcReceived: (Int) -> Unit   // 👈 callback en vez de MutableStateFlow
) {
    private var client: MqttAsyncClient? = null

    fun connect() {
        client = MqttAsyncClient(MqttConfig.MQTT_BROKER_URL, MqttConfig.CLIENT_APP, MemoryPersistence())
    ...
    private fun handleFcMessage(msg: MqttMessage) {
        ...
        // 1. Notificar al caller (MainActivity) que actualice el Repository
        onFcReceived(fcMsg.bpm)

        // 2. Re-publicar al topic TV
